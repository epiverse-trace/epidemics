...
#' @param connectivity_matrix A numeric matrix for the contact matrix between the 
#' elements of the populations
#' @param method The method used to combine the contact matrices of `populations`,
#' can be a character chain (`linear` or `gravity`) which will call an internal function, 
#' or a user-defined function with two arguments (`populations` and `connectivity_matrix`)
#' and returning a numeric matrix.
...
combine_populations <- function(populations, connectivity_matrix, 
  ...
  } else checkmate::assert_function(
    method, args = c("populations", "connectivity_matrix"))
  
  # Create demography vector for the combined population
  combined_demography_vector <- do.call(c, lapply(populations, extract_demography))
  # Create initial condition vector for the combined population
  combined_initial_condition <- do.call(rbind, lapply(populations, extract_initial))
...
#' 
#' @return A named vector corresponding to the demography vector, named after the
...
extract_demography <- function(population){
  ...
  # Rename demography as the population name, and the name of each element of demography
...
#' @param population A `<population>` object
#'
#' @return A named vector corresponding to the initial conditions, named after the
#' `name` element of the population object
#' @keywords internal
#' @noRd
extract_initial <- function(population){
  ...
  # Rename initial using the population name, and the name of each element of initial
...
extract_contact <- function(population){
  ...
  # Rename contact as the population name, and the name of each element of contact matrix
...
#'
#' @param populations A list of `<population>` objects
#' @param connectivity_matrix A numeric matrix for the proportion of contacts between 
...
combine_contact <- function(populations, connectivity_matrix){
  ...
  # Initial combined matrix, an squared matrix of size equal to the sum of the size 
  # of each contact matrix
  combined_matrix <- matrix(0, nrow = do.call(sum, lapply(contact_matrices, nrow)),
  ...
  for(i in seq_along(populations)){
    for(j in seq_along(populations)){
      ...
      # connectivity between the populations * connectivity between the sub-populations
...
gravity_contact <- function(populations, connectivity_matrix){
  ...
  # - Extract the demography vector and sum the number of inhabitants in all groups
  ...
  ## connectivity = distance ^(-1) * population_i * population_j
  for(i in seq_along(populations)){
    for(j in seq_along(populations)){
  ...
  for(i in seq_along(populations)){
    for(j in seq_along(populations)){
      index_row <-  1 + cumsum(n_group_per_pop)[i] - rev(seq_len(n_group_per_pop[i]))
      index_col <- 1 + cumsum(n_group_per_pop)[j] - rev(seq_len(n_group_per_pop[j]))
      
      # For each Each element of combined matrix is equal to
      # Number of contacts from group x (column of combined_matrix) to group y (row of combined_matrix) = 
      #   number of contacts from x to y in the population x belongs to *
      #   connectivity from the population x belongs to to the population y belongs to
      combined_matrix[index_row, index_col] <- 
        contact_matrices[[j]] * connectivity_matrix[i, j]
    }
  }
