...
- `.cross_check_timedep()` returns `x` if `x` is not `NULL`, otherwise
  ...
  by default; see
  [`.no_time_dependence()`](https://epiverse-trace.github.io/epidemics/dev/reference/dot-no_time_dependence.md);
...
- `.cross_check_popchange()` returns `x` after checks against
  ...
  [`.no_population_change()`](https://epiverse-trace.github.io/epidemics/dev/reference/dot-no_population_change.md).
