...
lambda_prod[, ] <- C[i, j] * I[j] * beta_t * contact_reduction_total[j] *
...
vax_rate[] <- if (S[i] == 0) {
  0
} else if (t >= vax_start[i] && t <= vax_end[i]) {
  vax_nu[i] / S[i]
} else {
  0
}
...
# ODEs
deriv(S[]) <- if (vax_rate[i] + lambda[i] <= 1) {
  -(lambda[i] + vax_rate[i]) * S[i]
} else {
  -S[i]
}
...
deriv(R[]) <- gamma_t * I[i]
deriv(V[]) <- if (vax_rate[i] + lambda[i] <= 1) {
  vax_rate[i] * S[i]
} else if (vax_rate[i] > 0) {
  S[i] * (1 - lambda[i])
} else {
  0
}
