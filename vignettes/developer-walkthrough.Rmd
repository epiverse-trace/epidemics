...
The package implements models using `odin`, a domain-specific language for differential equations that combines readability with computational efficiency. Odin automatically generates optimized R code, eliminating the need for compiled `C++` while maintaining performance. This simplifies development and ensures models are both easy to modify and fast to execute.

This vignette does not tackle questions around epidemic modelling choices - such as which compartmental transitions should be allowed; we strongly recommend the involvement of a specialist in such cases.
