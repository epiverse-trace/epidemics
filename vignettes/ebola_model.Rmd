...
initial_conditions <- c(
  S = population_size - 2, E = 1, I = 1, H = 0, F = 0, R = 0
) / population_size
```
...
We assume that ebola has a mean infectious period of 5 days, and that the time between exposure and symptom onset --- the pre-infectious period --- is also 5 days.
While ebola has very high severity, our model does not yet have an explicit "deaths" compartment so we assume that individuals in the "recovered" compartment are a composite of those who have recovered and those who have died.
Functionally, they are similar as they are not part of the model dynamics.
However, this will change in future iterations as we incorporate funeral transmission.
...
The model is run for 365 days, with data reported on each day.
...
We plot the ebola epidemic over time, showing only the number of individuals in the exposed and infectious compartments.
