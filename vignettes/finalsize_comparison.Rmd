---
title: "Comparing mechanistic and analytical methods for epidemic size estimation"
output:
...
link-citations: true
vignette: >
  %\VignetteIndexEntry{Comparing mechanistic and analytical methods for epidemic size estimation}
...
This leads to the following model parameters for transmissibility $\beta$, the rate of transition from exposed to infectious $\sigma$, and the recovery rate $\gamma$.

```{r}
# simulate pandemic parameters
transmissibility <- 1.5 / 7
...
An important reason to use _finalsize_ for final size calculations may be speed --- `finalsize::final_size()` is much faster than `epidemics::epidemic_size()` applied to `epidemics::model_default_cpp()`.
