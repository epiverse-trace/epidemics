...
> If `method` is set to gravity, a gravity model is used to estimate the number of connections between populations. For each population $i$ and $j$, the number of connections is computed as $C_{ij} = \frac{N_i * N_j}{d_{ij}}$, with $N_i$ and $N_j$ the overall number of inhabitants in each population, and $d_{ij}$ the distance between population (set to 1 if $i=j$), taken from the `connectivity_matrix` argument. To avoid large values, $C_{ij}$ is divided by the maximum value of $C$. The number of contacts between groups is computed as the product of $\frac{C_ij}{max(C)}$ and the number of contacts between groups in the population of origin.
