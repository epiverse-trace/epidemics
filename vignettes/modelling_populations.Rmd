...
We prepare two age-stratified populations: one using the contact data and demography from the U.K., and the other using the contact data and demography from Germany. The epidemiological compartments match the default epidemic model (SEIR-V).
...
We then use the function `combine_populations` to combine the two populations. The resulting population object will contain 6 stata (3 age groups in each population). If the method is defined as `"linear"`, the contact rate between group is computed as the product between the contact matrix between groups and the connectivity between populations. 
...
filter(output, compartment %in% "infectious") %>%
  mutate(region = gsub("[.].*", "", demography_group), 
         age = gsub(".*[.]", "", demography_group)) %>%
  ggplot(
    aes(
      x = time,
      y = value,
...
When combining populations, the argument method can be set to "gravity" to implement a gravity model instead, using the connectivity matrix as a distance between regions. In this example, the populations are 200km away. The gravity model then uses the overall number of inhabitants in each region and the distance to compute the connectivity between the two regions.
...
```{r class.source = 'fold-hide'}
# plot figure of epidemic curve
filter(output_gravity, compartment %in% "infectious") %>%
  mutate(region = gsub("[.].*", "", demography_group), 
         age = gsub(".*[.]", "", demography_group)) %>%
  ggplot(
    aes(
...
The argument `method` can also be defined as a function, which will be used to compute the connectivity matrix. Using `method = gravity_contact` instead of `method = "gravity` leads to the same output. 

```{r}
gravity_population_with_function <- combine_populations(
  populations = list(population1, population2),
  connectivity_matrix = distance_matrix,
...
output_gravity_with_function <- model_default(
  population = gravity_population_with_function,
  time_end = 1000, increment = 1.0
)
...
Each population is placed on a line, with consecutive populations being 50km away (i.e. populations 1 and 2 are 50km away, 1 and 3 are 100km away...).

```{r}
distance_matrix_n <- matrix(50 * abs(rep(seq_len(n), n) - rep(seq_len(n), each = n)), nrow = n)
```
...
The outbreak first peaks in region 5, then in region 4, 3, 2, and 1, with similar number of cases.

```{r class.source = 'fold-hide'}
filter(output_gravity_n, compartment %in% "infectious") %>%
  mutate(region = gsub("[.].*", "", demography_group), 
         age = gsub(".*[.]", "", demography_group)) %>%
  group_by(time, region) |> summarise(value = sum(value)) %>%
  ggplot(
    aes(
      x = time,
      y = value,
