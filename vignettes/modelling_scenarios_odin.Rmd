...
seirv_model <- odin::odin({
  ...
  contact_reduction[,] <- log(contact_reduction[i,j])
  contact_reduction_total[] <- exp(sum(contact_reduction[i,])) #exp(sum(log(contact_reduction[,j])))
  ...
  vax_rate[] <- 0 #if (t >= vax_start[i] && t < vax_end[i]) vax_nu[i] else 0
...
model_default_odin <- function(population,
  ...
  # DEBUG: population=uk_population; intervention=combined_intervention; vaccination = vaccinate_elders; time_end = 600
  ...
  # DEBUG intervention_effect = 0*intervention_effect
