...
vaccinate_elders <- vaccination(
  ...
  nu = matrix(c(0, 0, 0)) # assume zero for now
)
...
model_default_odin <- function(population,
  ...
  # TO DO: Make the default a matrix so that the odin model compiles correctly
...
data_baseline_infections <- new_infections(output, by_group = TRUE)
```
