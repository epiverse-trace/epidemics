---
title: "Modelling overlapping and sequential interventions on social contacts"
output:
...
pkgdown:
  as_is: true
vignette: >
  %\VignetteIndexEntry{Modelling overlapping and sequential interventions on social contacts}
...
::: {.alert .alert-warning}
**New to _epidemics_, or to modelling non-pharmaceutical interventions?** It may help to read the ["Get started"](epidemics.html) and ["Modelling a non-pharmaceutical intervention on social contacts"](modelling_interventions.html) vignettes first!
:::
...
data_baseline <- model_default_cpp(
...
```{r}
# no intervention baseline scenario
data_schools_closed <- model_default_cpp(
  population = uk_population,
...
This multiple intervention can then be passed --- as a single `intervention` object --- to  an epidemic model in `model_default_cpp()`.
...
::: {.alert .alert-primary}
- We can combine any number of `intervention` objects into a single `intervention` object to be passed to `model_default_cpp()`.
...
We simulate an epidemic using the `model_default_cpp()` function, calling the default model outlined in the ["Get started vignette"](epidemics.html).
...
We simulate an epidemic using the `model_default_cpp()` function for 600 days as before.
