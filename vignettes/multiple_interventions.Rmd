...
We simulate an epidemic using `epidemic_default_cpp()`, calling the default model outlined in the ["Get started vignette"](epidemics.html).

```{r}
# no intervention baseline scenario
data_baseline <- epidemic_default_cpp(
...
close_schools <- intervention(
  ...
  time_begin = 60,
  time_end = 60 + 180,
  contact_reduction = matrix(c(0.3, 0.01, 0.01))
)
...
```{r}
# no intervention baseline scenario
data_schools_closed <- epidemic_default_cpp(
  population = uk_population,
  infection = pandemic,
...
close_workplaces <- intervention(
  ...
  contact_reduction = matrix(c(0.01, 0.3, 0.01))
)
...
This multiple intervention can then be passed --- as a single `intervention` object --- to  an epidemic model in `epidemic_default_cpp()`.
...
::: {.alert .alert-primary}
- We can combine any number of `intervention` objects into a single `intervention` object to be passed to `epidemic_default_cpp()`.
...
We simulate an epidemic using the `epidemic_default_cpp()` function, calling the default model outlined in the ["Get started vignette"](epidemics.html).

We simulate the epidemic for 600 days.

```{r}
# get data from an epidemic model with both interventions
data_combined <- epidemic_default_cpp(
  population = uk_population,
  infection = pandemic,
...
We simulate an epidemic using the `epidemic_default_cpp()` function for 600 days as before.
