...
infection_data <- new_infections(data_combined, by_group = FALSE)
start_date <- filter(infection_data, new_infections > 50000) %>%
  filter(time == min(time)) %>%
  pull(time)
```

In the scenario with combined interventions, daily new infections are forecast to exceed 50,000 on the `r start_date`th day of the epidemic.
