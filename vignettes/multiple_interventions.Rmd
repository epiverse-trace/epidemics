...
We simulate an epidemic using the `epidemic()` function, calling the default model outlined in the ["Get started vignette"](epidemics.html).

```{r}
# no intervention baseline scenario
data_baseline <- epidemic(
...
::: {.alert .alert-primary}
- We can combine any number of `intervention` objects into a single `intervention` object to be passed to `epidemic()`.
...
We simulate an epidemic using the `epidemic()` function for 600 days as before.
