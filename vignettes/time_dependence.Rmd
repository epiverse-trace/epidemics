...
::: {.alert .alert-warning}
**New to _epidemics_?** It may help to read the ["Get started"](epidemics.html) vignette first, and to browse through some of the other vignettes such as ["Modelling a non-pharmaceutical intervention"](modelling_interventions.html), or ["Modelling interventions on epidemic parameters"](rate_interventions.html).
:::
...
This is intended to be different from [modelling interventions on epidemic parameters](rate_interventions.html) by allowing small, continuous changes to the epidemic parameters, while rate interventions are intended to be large step-changes as would be expected from policies such as mask mandates or the roll-out of therapeutics.
...
Next we prepare a function that affects the transmissibility (often denoted $\beta$) in such a way that there are two peaks and two troughs in the transmissibility rate over the course of a year.
...
Next we run an epidemic model while including the time dependence of transmissibility.

```{r}
# note transmissibility is the function/model parameter
# the function that affects it may have any name
data <- model_default_cpp(
  population = uk_population,
  time_dependence = list(
    transmissibility = mod_beta
  ),
...
Here, we can see that the epidemic has a small first wave, followed by a larger second wave later in the model run (which represents one year).
...
# get data from vaccination model output
data_vaccinations <- get_parameter(output_vax, "data") %>%
  filter(
    compartment == "vaccinated", demography_group == "40+"
  )
```
...
Here, we can see that over 7 million individuals are vaccinated (and immunised; blue line, right-hand Y axis) over the 100 days between the end of the first wave of infections, and the start of the second wave of infections.

Vaccination reduces the number of daily infections among individuals of all age groups in the second wave, with a much lower peak of daily new infections than the baseline scenario (dashed line); this could represent a substantial public health benefit.
